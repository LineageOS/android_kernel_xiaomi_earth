...
int fts_set_proximity_switch(int proximity_switch)
{
    ...
    FTS_DEBUG("[PROXIMITY]SENSOR_ENABLE value = %d", proximity_switch);
    /* Enable proximity */
    mutex_lock(&fts_data->input_dev->mutex);
    err = fts_enter_proximity_mode(proximity_switch);
...
/* C3T code for HQ-219022 by sunfeiting at 2022/08/29 end */
    ...
    /* 由于kernel msleep函数时间会飘，1ms有可能变成20ms，所以采用mdelay(2)
       如果实际量测发现不会飘很长，也可以不改，主要会影响FW下载时间
       msleep(1);*/
    mdelay(2);
...
/*C3T code for HQ-244359 by chenzimo at 2022/10/17 start*/
#if     FTS_PALM_EN
    enter_palm_mode(ts_data);
#endif
/*C3T code for HQ-244359 by chenzimo at 2022/10/17 end*/
...
static int fts_ts_resume(struct device *dev)
{
    ...
    /*C3T code for HQ-250558 by chenzimo at 2022/10/9 start*/
    fts_release_all_finger();
    /*C3T code for HQ-250558 by chenzimo at 2022/10/9 end*/
...
/*C3T code for HQ-219139 by chenzimo at 2022/8/17 start*/
#if FTS_PSENSOR_ENABLE
    if ((fts_proximity_resume() == 0) && (fts_proximity_suspend_flag == true)) {
/*C3T code for HQ-256531 by jiangyue at 2022/11/14 start*/
        if (ts_data->gesture_support){
            fts_gesture_resume(ts_data);
        }
/*C3T code for HQ-256531 by jiangyue at 2022/11/14 end*/
